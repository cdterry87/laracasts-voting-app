<nav class="hidden md:flex items-center justify-between text-xs text-gray-400">
    <ul class="flex  uppercase font-semibold border-b-4 pb-3 space-x-10">
        <li>
            <a href="{{ route('idea.index', ['status' => 'All']) }}"
               wire:click.prevent="setStatus('All')"
               class="border-b-4 pb-3 transition duration-150 ease-in hover:border-blue @if($status === 'All') border-blue text-gray-900 @endif">
                All Ideas ({{ $statusCount['all_statuses'] }})
            </a>
        </li>
        <li>
            <a href="{{ route('idea.index', ['status' => 'Considering']) }}"
        ...
        <li>
            <a href="{{ route('idea.index', ['status' => 'In Progress']) }}"
    ...
    <ul class="flex uppercase font-semibold border-b-4 pb-3 space-x-10">
        <li>
            <a href="{{ route('idea.index', ['status' => 'Implemented']) }}"
        ...
        <li>
            <a href="{{ route('idea.index', ['status' => 'Closed']) }}"
